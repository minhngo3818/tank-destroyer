...
class LaserCharge(Sprite):
	...
	def update(self):
		self.dx = self.x_gather - self.rect.x
		self.dy = self.y_gather - self.rect.y
		...
		if self.direction == "up" and self.rect.y >= self.y_gather - 20 :
			self.kill()
		elif self.direction == "down" and self.rect.y <= self.y_gather + 20:
			self.kill()
		elif self.direction == "left" and self.rect.x >= self.x_gather - 20:
			self.kill()
		elif self.direction == "right" and self.rect.x <= self.x_gather + 20: 
			self.kill()


		
		"""if self.direction == "up":
			self.rect.y += self.speed		# y does not update
			if self.dx > 0:
				self.rect.x += self.speed	# Cannot happen
			else:
				self.rect.x -= self.speed	# This case happen
...
def chargeArea(direction):
	...
	for i in range(density):
		particle = LaserCharge(screen, pos_x, pos_y, pos_gather_x, pos_gather_y, direction)
